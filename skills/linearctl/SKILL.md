...
- `linearctl project list [--team <id>] [--state <name>] [--all-teams] --json` — includes portfolio fields (`progress`, `health`, `currentProgress`), normalized `milestones`, and milestone truncation metadata (`milestonesPageInfo`, `milestonesTruncated`) in JSON output
