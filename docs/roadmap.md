...
- Agent skills (linear-cli, linear-raw-gql)
...
- AUR package (`linear-cli-bin`) for Arch Linux
- Homebrew formula for macOS
- Nix package
- Scoop manifest for Windows
- Windows binary builds (pending Bun Windows compile support)
- npm global install (`npm install -g @qwrobins/linear-cli`)
...
- `linear config get/set` commands for managing profile settings
- `linear init` interactive setup wizard
- Shell completions (bash, zsh, fish)
- `--output-format table` for human-readable tabular output
- `linear issue move <identifier> --state <name>` as alias for update
