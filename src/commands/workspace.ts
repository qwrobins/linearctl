...
async function handleWorkspaceList(options: WorkspaceCommandOptions): Promise<number> {
  ...
  if (options.jsonEnvelope) {
    ...
    if (workspaces.length === 0) {
      process.stdout.write("No workspaces configured. Run linear auth login to add one.\n");
...
export async function handleWorkspaceCommand(
  ...
  process.stderr.write("Error: unsupported workspace command. Try linear workspace list.\n");
  return ExitCode.ValidationError;
}
