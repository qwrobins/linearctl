...
function printApiHelp(manifest: ApiCommandManifest): void {
  ...
  process.stdout.write("linear api <resource> <operation>\n\n");
  ...
  process.stdout.write("\nUse: linear api <resource> --help for operations\n");
  process.stdout.write("Use: linear api search <term> to search commands\n");
}
...
function printResourceHelp(manifest: ApiCommandManifest, resource: string): void {
  ...
  if (entries.length === 0) {
    process.stderr.write(`Error: unknown resource '${resource}'. Use 'linear api --help' to list resources.\n`);
    return;
  }
  process.stdout.write(`linear api ${resource} <operation>\n\n`);
...
export async function handleApiCommand(
  ...
  if (manifest === null) {
    process.stderr.write(
      ...
      "Run 'linear schema pull' and then 'bun run generate:api-manifest' to generate it.\n"
    );
  ...
  if (resource === "search") {
    ...
    if (term === undefined || term === "") {
      process.stderr.write("Error: usage: linear api search <term>\n");
      return ExitCode.ValidationError;
    }
